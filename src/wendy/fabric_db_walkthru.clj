...
; Start from scratch each time this file is loaded, so the seed data below isn't transacted twice.
(d/delete-database db-uri)
(d/create-database db-uri)

; Connect to the transactor in same REPL used to create your database:

(def conn (d/connect db-uri))

;; conn holds your database connection.

@(d/transact conn [{:db/doc "Hiya, world! This is my fabric database."}])

;; =====  TRANSACT A SCHEMA

; Bind the schema vector to a var called fabric-schema:
...
(def fabric-schema [#_{:db/ident :fabric/name 
                    ...
                    ;; other types: challis, satin, denim, twill, chiffon, suiting, gabardine, 
...
;; retrieve the current db value and store it in a var:

(def db (d/db conn))
;; => #'wendy.fabric-db/db

;; Issue queries against the database value, using q.

;; Create a vector with the three mandatory components:
...
(def all-blue-plant-fabrics '[:find ?e
...
(def color-intensity-of-blue-plants '[:find ?e ?color-intensity
                 :where [?e :fabric/color :color/blue]
                        [?e :fabric/fiber-origin :fiber-origin/plant]
...
;; What do i have? 2 vectors, each with 1 fabric entity id and 1 color-intensity id

;; What do i want? color-intensity id as a keyword

;; eventually, I want to build a name from existing values.

;; what values do i want in my name?
...
; then i can put these in a string as a name.

; first, lets get turn these numbers into something more meaningful.
...
;; but what fabric is this? find out with pull.

;; d/pull takes a db, pattern, eid
...
;; but what are these ids? found out with touch.

;; d/touch takes an entity.
...
; What I have? all the attributes of 1 entity.
